...
- The filename input is pre-filled with `kanban.json` and shows a "Will be saved at: <abs path>" preview that updates as you type — pressing Enter creates the file at that path
...
- Choosing a file fully adopts that backend: the in-memory state is transferred to the new on-disk backend, the file is created, and subsequent changes are persisted normally
