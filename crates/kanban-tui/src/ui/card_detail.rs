...
pub(super) fn render_relationship_boxes(
    ...
    let all_cards: Vec<kanban_domain::Card> = app.render_data.cards_by_id.values().cloned().collect();
    ...
    if let Some(card) = app.get_card_for_detail_view() {
        ...
        if let Some(board_idx) = app.selection.active_board_index {
            if let Some(board) = app.render_data.boards.get(board_idx) {
                ...
                let parents = app.render_data.graph.cards.parents(card_id);
                let children = app.render_data.graph.cards.children(card_id);
                ...
                if has_sprint_logs {
                    ...
                    let meta_lines =
                        build_metadata_lines(card, board, &app.render_data.sprints, &app.app_config);
