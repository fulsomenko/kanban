...
fn test_apply_config_edit_save_failure_does_not_update_in_memory_config() {
    ...
    // Include configuration_location pointing to an unwritable path to force save() failure.
    // /dev/null is a character device, so create_dir_all("/dev/null") fails (not a dir).
