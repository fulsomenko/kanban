...
mod sqlite {
    ...
    async fn test_append_and_load_commands() {
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_load_commands_half_open_range() {
        let (store, _dir) = make_store().await;
        store.append_commands(&[make_cmd("B1")]).unwrap();
        store.append_commands(&[make_cmd("B2")]).unwrap();
        store.append_commands(&[make_cmd("B3")]).unwrap();
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_truncate_commands_after() {
        let (store, _dir) = make_store().await;
        store.append_commands(&[make_cmd("B1")]).unwrap();
        store.append_commands(&[make_cmd("B2")]).unwrap();
        store.append_commands(&[make_cmd("B3")]).unwrap();
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_truncate_commands_after_zero_clears_all() {
        let (store, _dir) = make_store().await;
        store.append_commands(&[make_cmd("B1")]).unwrap();
        store.append_commands(&[make_cmd("B2")]).unwrap();
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_shift_commands_renumbers() {
        let (store, _dir) = make_store().await;
        store.append_commands(&[make_cmd("B1")]).unwrap();
        store.append_commands(&[make_cmd("B2")]).unwrap();
        store.append_commands(&[make_cmd("B3")]).unwrap();
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_store_and_load_snapshot_at() {
        let (store, _dir) = make_store().await;
        assert!(
            store.supports_indexed_snapshots(),
            "SqliteStore should support indexed snapshots"
        );

        // SQLite stores snapshots in the command_log table, so a command
        // must exist at the index before a snapshot can be attached.
        store.append_commands(&[make_cmd("B1")]).unwrap();
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_shift_commands_removes_old_snapshots() {
        let (store, _dir) = make_store().await;
        store.append_commands(&[make_cmd("B1")]).unwrap();
        store.append_commands(&[make_cmd("B2")]).unwrap();
        store.append_commands(&[make_cmd("B3")]).unwrap();
